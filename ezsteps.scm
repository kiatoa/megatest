...
(define (ezsteps:run-from testdat start-step-name run-one)
  (let* ((test-run-dir  ;; (filedb:get-path *fdb* 
	 ...
	 (run-id        (dbr:test-run_id testdat))
	...
	(begin
	  ...
	  ;;
	  (let* ((item-path (dbr:test-item-path testdat)) ;; (item-list->path itemdat))
		 (testinfo  (rmt:get-testinfo-by-id run-id test-id))) ;; refresh the testdat, call it iteminfo in case need prev/curr
	    ;; Am I completed?
	    (if (equal? (dbr:test-state testinfo) "RUNNING") ;; (not (equal? (dbr:test-state testinfo) "COMPLETED"))
		(let ((new-state  (if kill-job "KILLED" "COMPLETED") ;; (if (eq? (vector-ref exit-info 2) 0) ;; exited with "good" status
		  ...
		  ;; (debug:print-info 2 "Test NOT logged as COMPLETED, (state=" (dbr:test-state testinfo) "), updating result, rollup-status is " rollup-status)
		  (debug:print-info 2 *default-log-port* "Test NOT logged as COMPLETED, (state=" (db:test-state testinfo) "), updating result, rollup-status is " rollup-status)
