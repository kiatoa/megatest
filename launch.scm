	    ...
	    (let* ((item-path (item-list->path itemdat))
		  ...
		  (let ((new-state  (if kill-job? "KILLED" "COMPLETED") ;; (if (eq? (vector-ref exit-info 2) 0) ;; exited with "good" status
			...
			(new-status (cond
				     ...
				     (else "FAIL")))) ;; (dbr:test-status testinfo)))
		    (debug:print-info 1 "Test exited in state=" (dbr:test-state testinfo) ", setting state/status based on exit code of " (launch:einf-exit-status exit-info) " and rollup-status of " (launch:einf-rollup-status exit-info))
