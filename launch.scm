...
(define (launch:execute encoded-cmd)
	...
	(let* ((testpath  (assoc/default 'testpath  cmdinfo))  ;; testpath is the test spec area
	  ...
	  (let* ((test-info (rmt:get-test-info-by-id run-id test-id))
	    ...
	    (cond
	     ((member (dbr:test-state test-info) '("INCOMPLETE" "KILLED" "UNKNOWN" "KILLREQ" "STUCK")) ;; prior run of this test didn't complete, go ahead and try to rerun
	     ...
	     ((not (member (dbr:test-state test-info) '("REMOVING" "REMOTEHOSTSTART" "RUNNING" "KILLREQ")))
	     ;; ((member (db:test-get-state test-info) '("RUNNING" "REMOTEHOSTSTART"))
	      ...
	      (tests:test-force-state-status! run-id test-id "REMOTEHOSTSTART" "n/a"))
	     (else ;; (member (dbr:test-state test-info) '("REMOVING" "REMOTEHOSTSTART" "RUNNING" "KILLREQ"))
	      ;; (debug:print 0 "ERROR: test state is " (dbr:test-state test-info) ", cannot proceed")
	  ...
	  (let* ((m            (make-mutex))
	    ...
	    (let* ((item-path (item-list->path itemdat))
	      ...
	      ;; Am I completed?
	      (if (member (dbr:test-state testinfo) '("REMOTEHOSTSTART" "RUNNING")) ;; NOTE: It should *not* be REMOTEHOSTSTART but for reasons I don't yet understand it sometimes gets stuck in that state ;; (not (equal? (dbr:test-state testinfo) "COMPLETED"))
		  (let ((new-state  (if kill-job? "KILLED" "COMPLETED") ;; (if (eq? (vector-ref exit-info 2) 0) ;; exited with "good" status
				                                        ;; "COMPLETED"
							                ;; (dbr:test-state testinfo)))   ;; else preseve the state as set within the test
				    )
			(new-status (cond
				     ...
				     ((eq? (launch:einf-rollup-status exit-info) 0)     ;; (vector-ref exit-info 3)
				      ...
				      (if (equal? (dbr:test-status testinfo) "AUTO") "AUTO" "PASS"))
				     ...
				     ((eq? (launch:einf-rollup-status exit-info) 2)	     ;;	(vector-ref exit-info 3)
				      ...
				      (if (equal? (dbr:test-status testinfo) "AUTO") "AUTO-WARN" "WARN"))
		    ...
		    (debug:print-info 1 *default-log-port* "Test exited in state=" (db:test-state testinfo) ", setting state/status based on exit code of " (launch:einf-exit-status exit-info) " and rollup-status of " (launch:einf-rollup-status exit-info))
...
(define (launch-test test-id run-id run-info keyvals runname test-conf test-name test-path itemdat params)
  ...
  (let* ((tregistry       (tests:get-all))
    ...
    (if (and (args:get-arg "-preclean") ;; user has requested to preclean for this run
	     (not (member (dbr:test-rundir testinfo)(list "n/a" "/tmp/badname")))) ;; n/a is a placeholder and thus not a read dir
	(begin
	  ;; (debug:print-info 0 "attempting to preclean directory " (dbr:test-rundir testinfo) " for test " test-name "/" item-path)
	  (debug:print-info 0 *default-log-port* "attempting to preclean directory " (dbr:test-rundir testinfo) " for test " test-name "/" item-path)
