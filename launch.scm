...
(define (launch:setup #!key (force-reread #f) (areapath #f))
  ...
  (if (and *toppath*
	...
	*toppath*)
      (let ((res (launch:setup-body-modified force-reread: force-reread areapath: areapath)))
	(mutex-unlock! *launch-setup-mutex*)
	res)))

(define (launch:setup-body-modified #!key (force-reread #f) (areapath #f))
...
(define (launch:setup-body-orig #!key (force-reread #f) (areapath #f))
  (if (and (eq? *configstatus* 'fulldata)
	   *toppath*
	   (not force-reread)) ;; no need to reprocess
      *toppath*   ;; return toppath
