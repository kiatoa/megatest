    ...
    (apply iup:vbox  ; #:expand "YES"
	   (list 
	    (store-label "testname"
			 (iup:label (dbr:test-testname  testdat) #:expand "HORIZONTAL")
			 (lambda (testdat)(dbr:test-testname testdat)))
	    (store-label "item-path"
			 (iup:label (dbr:test-item-path testdat) #:expand "HORIZONTAL")
			 (lambda (testdat)(dbr:test-item-path testdat)))
	    (store-label "teststate" 
			 (iup:label (dbr:test-state testdat) #:expand "HORIZONTAL")
			 (lambda (testdat)
			   (dbr:test-state testdat)))
	    (let ((lbl   (iup:label (dbr:test-status testdat) #:expand "HORIZONTAL")))
	      (hash-table-set! widgets "teststatus"
			       (lambda (testdat)
				 (let ((newstatus (dbr:test-status testdat))
				       (oldstatus (iup:attribute lbl "TITLE")))
				   (if (not (equal? oldstatus newstatus))
				       (begin
					 (iup:attribute-set! lbl "FGCOLOR" (car (gutils:get-color-for-state-status (dbr:test-state testdat)
														   (dbr:test-status testdat))))
					 (iup:attribute-set! lbl "TITLE" (dbr:test-status testdat)))))))
	      lbl)
	    (store-label "testcomment"
			 (iup:label "TestComment                             "
				    #:expand "HORIZONTAL")
			 (lambda (testdat)
			   (let ((newcomment (dbr:test-comment testdat)))
	    ...
	    (store-label "testdate" 
			 (iup:label "TestDate                           "
				    #:expand "HORIZONTAL")
			 (lambda (testdat)
			   (seconds->work-week/day-time (dbr:test-event_time testdat))))
	    )))))
...
(define (run-info-panel db keydat testdat runname)
  (let* ((run-id     (dbr:test-run_id testdat))
	   ...
	   (list
	    ...
	    (store-label "HostName"
			 (iup:label ;; (sdb:qry 'getstr 
			  (dbr:test-host testdat) ;; )
			  #:expand "HORIZONTAL")
			 (lambda (testdat)(dbr:test-host testdat)))
	    (store-label "DiskFree"
			 (iup:label (conc (dbr:test-diskfree testdat)) #:expand "HORIZONTAL")
			 (lambda (testdat)(conc (dbr:test-diskfree testdat))))
	    (store-label "CPULoad"
			 (iup:label (conc (dbr:test-cpuload testdat)) #:expand "HORIZONTAL")
			 (lambda (testdat)(conc (dbr:test-cpuload testdat))))
	    (store-label "RunDuration"
			 (iup:label (conc (seconds->hr-min-sec (dbr:test-run_duration testdat))) #:expand "HORIZONTAL")
			 (lambda (testdat)(conc (seconds->hr-min-sec (dbr:test-run_duration testdat)))))
	    (store-label "LogFile"
			 (iup:label (conc (dbr:test-final_logf testdat)) #:expand "HORIZONTAL")
			 (lambda (testdat)(conc (dbr:test-final_logf testdat))))
	    (store-label "ProcessId"
			 (iup:label (conc (dbr:test-process_id testdat)) #:expand "HORIZONTAL")
			 (lambda (testdat)(conc (dbr:test-process_id testdat))))
	    (store-label "Uname"
			 (iup:label "                                                   " #:expand "HORIZONTAL") ;;  #:wordwrap "YES")
			 (lambda (testdat) ;; (sdb:qry 'getstr 
			   (dbr:test-uname testdat))) ;; )
	    )))))
...
(define (update-state-status-buttons testdat)
  (let* ((state  (dbr:test-state  testdat))
	 (status (dbr:test-status testdat))
     ...
     (iup:vbox
      (iup:hbox (iup:label "Comment:")
		(let ((txtbox (iup:textbox #:action (lambda (val a b)
						      ...
						      (rmt:test-set-state-status-by-id run-id test-id #f #f b)
						      (set! newcomment b))
					   #:value (dbr:test-comment testdat)
					   #:expand "HORIZONTAL")))
		  (set! wtxtbox txtbox)
		  txtbox))
	     ...
	     (let* ((btns  (map (lambda (state)
				  (let ((btn (iup:button state
							 ...
							 #:action (lambda (x)
								    ...
								    (dbr:test-state-set! testdat state)))))
				    btn))
							 ...
							 #:action (lambda (x)
								    (let ((t (iup:attribute x "TITLE")))
									  ...
									  (begin
									    ...
									    (dbr:test-status-set! testdat status))))))))
				    btn))
...
(define (dashboard-tests:waiver run-id testdat ovrdval cmtcmd)
  (let* ((wpatt (configf:lookup *configdat* "setup" "waivercommentpatt"))
	 ...
	 (comnt (iup:textbox #:action (lambda (val a b)
			     ...
			     #:value (if ovrdval ovrdval (dbr:test-comment testdat))
			     #:expand "HORIZONTAL"))
	 (dlog  #f))
    (set! dlog (iup:dialog ;; #:close_cb (lambda (a)(exit)) ; #:expand "YES"
		...
		(iup:vbox ; #:expand "YES"
		 (iup:label (conc "Enter justification for waiving test "
				  (dbr:test-testname testdat)
				  (if (equal? (dbr:test-item-path testdat) "") 
				      ""
				      (conc "/" (dbr:test-item-path testdat)))))
...
(define (examine-test run-id test-id) ;; run-id run-key origtest)
  (let* ((db-path       (db:dbfile-path run-id)) ;; (conc (configf:lookup *configdat* "setup" "linktree") "/db/" run-id ".db"))
	...
	(begin
	  ...
	  (exit 1))
	(let* (;; (run-id        (if testdat (dbr:test-run_id testdat) #f))
	       ...
	       (testfullname  (if testdat (dbr:test-fullname testdat) "Gathering data ..."))
	       (testname      (if testdat (dbr:test-testname testdat) "n/a"))
	       ...
	       (testconfig    (begin
				...
				(handle-exceptions
				 exn
				 (tests:get-testconfig (dbr:test-testname testdat) test-registry #f)
				 (tests:get-testconfig (dbr:test-testname testdat) test-registry #t))))
	       (viewlog    (lambda (x)
	       ...
	       (refreshdat (lambda ()
				...
				((and need-update newtestdat)
				 ...
				 (set! logfile      (conc (dbr:test-rundir testdat) "/" (dbr:test-final_logf testdat)))
				 (set! rundir       ;; (filedb:get-path *fdb* 
				       (dbr:test-rundir testdat)) ;; )
				 (set! testfullname (dbr:test-fullname testdat))
				...
				(need-update ;; if this was true and yet there is no data ....
				 (dbr:test-testname-set! testdat "DEAD OR DELETED TEST")))
			       (if need-update
				   (begin
