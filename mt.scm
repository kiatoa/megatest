...
(define (mt:process-triggers run-id test-id newstate newstatus)
  (let* ((test-dat      (rmt:get-test-info-by-id run-id test-id)))
	...
	(let* ((test-rundir   ;; (rmt:sdb-qry 'getstr ;; (filedb:get-path *fdb*
		(dbr:test-rundir test-dat)) ;; ) ;; )
	       (test-name     (dbr:test-testname test-dat))
	       (tconfig       #f)
	       (state         (if newstate  newstate  (dbr:test-state  test-dat)))
	       (status        (if newstatus newstatus (dbr:test-status test-dat))))
	  (if (and test-rundir   ;; #f means no dir set yet
	      ...
	      (call-with-environment-variables
	       (list (cons "MT_TEST_NAME" test-name)
		     ...
		     (cons "MT_ITEMPATH"     (dbr:test-item-path test-dat)))
	       (lambda ()
