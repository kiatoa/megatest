...
(if (args:get-arg "-step")
	...
	(let* ((step      (args:get-arg "-step"))
	  ...
	  (if (not (setup-for-run))
		...
		(exit 1)))
	  (if (and state status)
	      (db:teststep-set-status! test-id step state status (args:get-arg "-m") logfile)
	      (begin
	...
	(let* ((startingdir (current-directory))
	  ...
	  (if (args:get-arg "-runstep")
		  ...
		  (let* ((stepname   (args:get-arg "-runstep"))
		    ...
		    (db:teststep-set-status! test-id stepname "start" "n/a" (args:get-arg "-m") logfile)
		    ;; run the test step
		    ...
		    (let ((msg (args:get-arg "-m")))
		      (db:teststep-set-status! test-id stepname "end" exitstat msg logfile))
		    )))
