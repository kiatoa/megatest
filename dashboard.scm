...
(define (compare-tests test1 test2)
  (let* ((test-name1  (dbr:test-testname  test1))
	 (item-path1  (dbr:test-item-path test1))
	 (eventtime1  (dbr:test-event_time test1))
	 (test-name2  (dbr:test-testname  test2))
	 (item-path2  (dbr:test-item-path test2))
	 (eventtime2  (dbr:test-event_time test2))
...
(define (get-itemized-tests test-dats)
  (let ((tnames '()))
    (for-each (lambda (tdat)
		(let ((tname (vector-ref tdat 0))  ;; (dbr:test-testname tdat))
		      (ipath (vector-ref tdat 1))) ;; (dbr:test-item-path tdat)))
		  (if (not (equal? ipath ""))
		      (if (and (list? tnames)
			       (string? tname)
...
(define (bubble-up test-dats #!key (priority 'itempath))
  (if (null? test-dats)
	...
	(let* ((tnames   '())                ;; list of names used to reserve order
	  ...
	  (for-each 
	   (lambda (testdat)
	     (let* ((tname (vector-ref testdat 0))  ;; dbr:test-testname testdat))
		    (ipath (vector-ref testdat 1))) ;; dbr:test-item-path testdat)))
		...
		(if buttondat
		    (let* ((test       (let ((matching (filter 
					 ...
					 (if (null? matching)
					     (vector -1 -1 "" "" "" 0 "" "" 0 "" "" "" 0 "" "")
					     (car matching))))
			   (testname   (dbr:test-testname  test))
			   (itempath   (dbr:test-item-path test))
			   (testfullname (test:test-get-fullname test))
			   (teststatus (dbr:test-status   test))
			   (teststate  (dbr:test-state    test))
			   ;;(teststart  (dbr:test-event_time test))
			   ;;(runtime    (dbr:test-run_duration test))
			   (buttontxt  (cond
	       ...
	       (butn       (iup:button "" ;; button-key 
				       ...
				       #:action (lambda (x)
						  (let* ((toolpath (car (argv)))
							 ...
							 (test-id  (dbr:test-id (vector-ref buttndat 3)))
							 (run-id   (dbr:test-run_id (vector-ref buttndat 3)))
