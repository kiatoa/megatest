...
(define (get-itemized-tests test-dats)
  (let ((tnames '()))
    (for-each (lambda (tdat)
		(let ((tname (vector-ref tdat 0))  ;; (db:test-testname tdat))
		      (ipath (vector-ref tdat 1))) ;; (db:test-item-path tdat)))
		  (if (not (equal? ipath ""))
		      (if (and (list? tnames)
			       (string? tname)
...
(define (bubble-up test-dats #!key (priority 'itempath))
  (if (null? test-dats)
	...
	(let* ((tnames   '())                ;; list of names used to reserve order
	  ...
	  (for-each 
	   (lambda (testdat)
	     (let* ((tname (vector-ref testdat 0))  ;; db:test-testname testdat))
		    (ipath (vector-ref testdat 1))) ;; db:test-item-path testdat)))
