...
(define (runs:rollup-run keys runname user keyvals)
  ...
  (let* ((db              #f)
    ...
    (for-each 
	 ...
	 (debug:print 4 *default-log-port* "Copying records in test_steps from test_id=" (dbr:test-id testdat) " to " (dbr:test-id new-testdat))
	 (cdb:remote-run ;; to be replaced, note: this routine is not used currently
	  (lambda ()
	    ...
	    (debug:print 4 *default-log-port* "Copying records in test_data from test_id=" (dbr:test-id testdat) " to " (dbr:test-id new-testdat))
	    (sqlite3:execute 
	     db 
