...
(define (archive:run-bup archive-command run-id run-name tests rp-mutex bup-mutex)
  ...
  (let* ((min-space    (string->number (or (configf:lookup *configdat* "archive" "minspace") "1000")))
	...
	(begin
	  (debug:print 0 #f "FATAL: No archive disks found. Please add disks with at least " min-space " MB space to the [archive-disks] section of megatest.config")
	  (debug:print 0 #f "       use [archive] minspace to specify minimum available space")
	  (debug:print 0 #f "   disks: " (string-intersperse (map cadr (archive:get-archive-disks)) "\n         "))
	  (exit 1))
    ...
    (for-each
     (lambda (test-dat)
 	 ...
 	 (cond
	  (toplevel/children
	   (debug:print 0 #f "WARNING: cannot archive " test-name " with id " test-id " as it is a toplevel test with children"))
	  ((not (file-exists? test-path))
	   (debug:print 0 #f "WARNING: Cannot archive " test-name "/" item-path " as path " test-path " does not exist"))
	  (else
	   (debug:print 0 #f
    ...
    (for-each 
     (lambda (disk-group)
       (debug:print 0 #f "Processing disk-group " disk-group)
